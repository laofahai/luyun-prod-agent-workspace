...
- 最终输出必须遵守 `OUTPUT_CONTRACT.md` 的 `luyun_support_investigation_v1` JSON 契约。
- `summary`、`facts`、`next_actions` 的每条内容都必须引用证据编号；没有证据的内容只能放 `unknowns`。
- 服务端 Agent 负责生成最终答复和证据包；用户端 WorkBuddy/ops-support skill 只能展示和转交，不得新增事实、原因或处理建议。
...
最终回答必须是 `OUTPUT_CONTRACT.md` 定义的 JSON 对象，且 `schema` 必须为 `luyun_support_investigation_v1`。

不要贴长源码、长日志、敏感数据、密钥或完整 traceback。必要时只给短摘要和位置。
